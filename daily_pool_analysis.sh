...
# ============================================================
#  每日股票池分析脚本
#  用法: ./daily_pool_analysis.sh [POOL]
#  说明: 分析指定股票池 Top 20 + 自选股，生成报告和图表
#
#  参数:
#    POOL - 股票池代码，可选值：
#           【指数池】sz50, hs300, zz500, cyb
#           【互联互通】hgt, sgt
#           【热门概念】newenergy, chip, tech, ai, ev
#           默认: hgt (沪股通)
#
#  示例:
#    ./daily_pool_analysis.sh              # 默认分析沪股通
#    ./daily_pool_analysis.sh chip         # 分析芯片板块
#    ./daily_pool_analysis.sh newenergy    # 分析新能源板块
# ============================================================
...
# 验证参数
VALID_POOLS="sz50 hs300 zz500 cyb hgt sgt newenergy chip tech ai ev"
if ! echo "$VALID_POOLS" | grep -wq "$POOL"; then
    echo "❌ 错误: 无效的股票池 '$POOL'"
    echo ""
    echo "可用的股票池:"
    echo "  【指数池】    sz50, hs300, zz500, cyb"
    echo "  【互联互通】  hgt, sgt"
    echo "  【热门概念】  newenergy, chip, tech, ai, ev"
    echo ""
    echo "用法: $0 [POOL]"
    echo "示例: $0 chip"
    exit 1
...
echo "============================================================"
echo "  每日股票分析 - ${POOL_NAME}  $(date '+%Y-%m-%d %H:%M')"
...
echo "📊 [1/3] ${POOL_NAME}潜力股发现 (Top ${TOP_N}, >= ${MIN_SCORE}分)"
echo "------------------------------------------------------------"
...
echo ""
echo "============================================================"
echo "  ${POOL_NAME} 分析完成!  $(date '+%H:%M:%S')"
